...
//                .fetchFirst();
    ...
    public void sort() {
        ...
        // 5 -> 6 -> null (null 은 마지막으로 이동하기 때문에)
...
//                .where(team.name.eq("teamA")) // inner 조인이라면 결과가 같으므로 where와 같다. 그러므로 left join일때만 on절을 사용하자
                .fetch();
    ...
    // setter 를 통해서 값이 들어간다. -> dto 에 setter 가 정의되어있지 않으면, 에러가 발생한다.
    @Test
    ...
    // dto 의 속성 변수들과 아래 constructor(MemberDto.class, "", "") 부분의 순서와 타입명이 일치해야한다.
    // 생성자를 통해서 값이 주입된다.
    @Test
    ...
    // ExpressionUtils : subQuery 를 사용해야할 때
    @Test
    public void findUserDto() {
        QMember memberSub = new QMember("memberSub");
        List<UserDto> result = queryFactory
                .select(Projections.fields(UserDto.class,
                        member.username.as("name"),  // entity 와 dto 의 명이 다를 때, as("원래변수명") 을 적어준다.
//                        member.age,
...
//                        member.username)))
                .where(member.username.eq(member.username.lower())) // lower 같은 ansi 표준 함수들은 querydsl이 상당부분 내장하고 있다. 따라서 다음과 같이 처리해도 결과 는 같다
                .fetch();
